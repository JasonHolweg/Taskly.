...
- **KI-Assets (Recraft):** `tools/recraft_gen.py` — Key in `.recraft_key` (gitignored). Items: Prompt-Tabelle in `journey_assets.md` ergänzen → `items-manifest` → `generate --manifest manifests/items.json` (überspringt Existierendes) → `process` (1024→256 px nach `public/assets/img/items/`) → rsync. Tanuki-Skins: Custom Style `tanuki` (ID in `tools/recraft_styles.json`, trainiert auf bestehenden Posen) + `tools/manifest_tanuki_beispiel.json` (3 Posen pro Outfit) → Ergebnis wie gehabt über die tanuki-raw-Pipeline. Szenen: `scenes-manifest` (9:16 Karten + Splash).
